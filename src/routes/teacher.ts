...
import { persistAndNotify, notifyUsers } from "../ws/emit";
...
// Lesson chat: list messages for this booking (teacher must own the booking)
teacherRouter.get(
...
// Lesson chat: send message (teacher), then notify student via WebSocket
teacherRouter.post(
  "/bookings/:id/messages",
  asyncHandler(async (req, res) => {
    const bookingIdStr = req.params.id;
    ...
    const participant = await getBookingChatParticipant(new Types.ObjectId(bookingIdStr), req.user!.id);
    if (!participant.allowed) return res.status(404).json({ error: "Booking not found" });
    if (participant.myRole !== "teacher") return res.status(403).json({ error: "Forbidden" });
